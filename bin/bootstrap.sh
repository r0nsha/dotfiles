...
if [ "$MACHINE" = "linux" ]; then
	...
	mkdir -p $GTK_THEMES
	sudo bash -c "$DOTFILES/gtk/Kanagawa-GTK-Theme/themes/install.sh --dest $GTK_THEMES --name Kanagawa --theme default --size compact --tweaks dragon black float"
	success "installed GTK theme"

	running "installing GTK icons..."
	GTK_ICONS=$HOME/.local/share/icons
	mkdir -p $GTK_ICONS
	cp -r $DOTFILES/gtk/kora/kora* $GTK_ICONS/.
