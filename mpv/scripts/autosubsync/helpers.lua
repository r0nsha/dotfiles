...
function self.file_exists(filepath)
  if not self.is_empty(filepath) then
    ...
    if info and info.is_file then return true end
  end
  return false
end
...
function self.find_executable(name)
  ...
  for _, path in pairs(self.alt_dirs()) do
    ...
    if self.file_exists(exec_path) then return exec_path end
  end
  return name
end

function self.is_path(str) return not not string.match(str, "[/\\]") end
