...
---@param side string "right" | "left"
function M.close_windows_on_side(side)
...
---@param cmd string|false|nil The command to run in the side panel
function M.open_side_panel(cmd)
  local current_win = vim.api.nvim_get_current_win()
  local current_tab = vim.api.nvim_win_get_tabpage(current_win)
  local wins = vim.api.nvim_tabpage_list_wins(current_tab)
